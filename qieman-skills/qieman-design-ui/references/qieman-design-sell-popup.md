...
# 然后阅读 references/qieman-sell-popup-design.md
```
...
`qieman-sell-popup-design` 是 **qieman-design-ui（L0）下的 L1 场景扩展规范**，文件位于 `references/qieman-sell-popup-design.md`。它定义 App 内卖出、赎回、退出策略、暂停计划等关键动作前的 **轻量确认与理性陪伴弹窗**，不是独立 openskills 目录。
