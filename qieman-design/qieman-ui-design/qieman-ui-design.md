...
qieman ui design skill 是一个面向 **盈米基金 / 且慢** 金融产品设计任务的高保真界面生成规范。它用于把复杂的基金投顾、账户复盘、资产配置、财富规划、策略介绍、优惠券营销和数据可视化需求，转化为清晰、克制、可信、可落地的 HTML 原型或设计稿结构。
...
| 层级 | 标识 | 入口 / 文件 | 职责 |
|------|------|-------------|------|
| **L0 基础层** | `qieman-ui-design` | 本目录 `SKILL.md` → `qieman-ui-design.md` | 品牌色、字体、间距、圆角、阴影、`components`、Remix Icon、通用页面 Pattern、输出与合规约束 |
...
| spec-id | 文件 | 适用任务 |
|---------|------|----------|
| `qieman-sell-popup-design` | [qieman-sell-popup-design.md](./qieman-sell-popup-design.md) | App 内 **卖出 / 赎回 / 退出策略** 底部全宽挽留弹窗（375×812、无关闭按钮、主题渐变） |

> L1 规范均在 frontmatter 声明 `extends: qieman-ui-design`。与 L0 冲突时：**几何尺寸、弹窗交互以 L1 为准**；品牌主色、字体气质、风险表达以 L0 为准。

### L2 独立场景（兄弟 skill）

| skill（文件夹 = `name`） | 与 L0 关系 | 典型产出 |
|--------------------------|------------|----------|
| `qieman-h5-design` | 叠加 L0 | App 内营销 H5、活动页、策略介绍页 |
| `qieman-vip-design` | `extends` L0 | 9:16 高净值私域海报、九宫格 |
| `qieman-report-ppt-design` | 对齐 L0 色板 | HTML 16:9 投顾报告幻灯片 |
| `qieman-pdf-design` | 对齐 L0 色板 | A4 / 可打印 PDF 报告 |
| `qieman-ppt-design` | 对齐 L0 色板 | PowerPoint 原生演示稿 |
| `qieman-data-visualization` | 图表色可引用 L0 `chart-*` | 独立图表与可视化 |

### 选用流程

1. **标准 App 页 / 看板 / 表单 / 列表** → 仅 **L0** `qieman-ui-design`。
2. **卖出挽留、赎回确认等底部弹窗** → **L0 + L1** `qieman-sell-popup-design`。
3. **营销 H5 / 活动长页** → **L0 + L2** `qieman-h5-design`（H5 内若含卖出弹窗，再叠 **L1**）。
4. **VIP 私域海报** → **L0 + L2** `qieman-vip-design`。
5. **客户报告幻灯片 / PDF / PPTX** → 对应 **L2** report-ppt / pdf / ppt-design；品牌色与 L0 对齐。

```bash
npx openskills read qieman-ui-design
# L1 扩展：阅读同目录 qieman-sell-popup-design.md（无独立 openskills 目录）
# L2 示例：
...
- **底部挽留 / 卖出确认弹窗** → 使用 L1 扩展 [qieman-sell-popup-design.md](./qieman-sell-popup-design.md)，不要仅在 L0 临时拼凑弹窗样式。
