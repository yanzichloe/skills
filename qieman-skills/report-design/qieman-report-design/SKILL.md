...
需要 **浏览器全屏演示、guizang 式横向翻页交互**（`100vw×100vh`、键盘/滚轮/触屏/圆点/ESC 索引），但视觉仍用本 skill 的报告 token 时，**叠加** L1 扩展：

| Skill | 画布 | 交互 |
|-------|------|------|
| **`qieman-report-design`（本文件）** | `720pt×405pt` 固定 | 垂直合并 + `fitDeck()` · **PDF/PPT 导出还原** |
| **`qieman-report-swipe-design`** | `100vw×100vh` 全屏 | guizang 横向翻页 · **现场演示** |

```bash
npx openskills read qieman-report-swipe-design
```

规范：`../qieman-report-swipe-design/SKILL.md` · 模板：`../qieman-report-swipe-design/assets/template-report-swipe.html`

**路由**：导出/合并 deck → 本 skill；演示/翻页 → `qieman-report-swipe-design`。两者并存，勿用翻页版覆盖 720pt 产出。

## Known Gaps

- 本规范主要面向 16:9 HTML 幻灯片，不覆盖移动端 H5。
- 不定义复杂交互、悬浮态、点击态和动效（横向翻页见 `qieman-report-swipe-design`）。
