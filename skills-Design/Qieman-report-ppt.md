---
version: alpha
name: qieman-wealth-report-ppt
description: 且慢财富报告 HTML 幻灯片（16:9）设计规范。用于生成可还原为 PPT 的财富诊断、账户复盘、目标规划、资产配置、投顾建议等报告型幻灯片。视觉以浅蓝报告画布、无描边白色信息卡片、克制金融图表、清晰页眉页脚为核心；强调专业、可信、安静、可读和 PPT 级还原。
globs: "生成的界面/**/*.html"
alwaysApply: false
...
# 且慢财富报告幻灯片 HTML Skill

## Overview

且慢财富报告幻灯片是一套 **PPT 还原优先** 的 HTML 报告设计语言，服务于账户诊断、周/月度复盘、财富目标规划、资产配置建议、投顾策略说明和高净值客户报告。它不是营销 H5，也不是后台数据看板，而是面向客户阅读与顾问讲解的 16:9 报告页。
...
- 所有白色内容区使用 `{component.panel}` / `{component.content-card}`，强制 `border: none`。
...
- 使用 `{component.panel}` 和 `{component.content-card}` 承载白色内容区。
- 保持 `.title` 为 18pt / 600。
- 保持页脚声明为 6.5pt 单行。
...
3. 优先引用 token：`{colors.*}`、`{typography.*}`、`{component.*}`，不要随意写新颜色。
4. 每次只调整一个组件，例如 `{component.kpi-card}` 或 `{component.chart-card}`。
